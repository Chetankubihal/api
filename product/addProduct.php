...
// make sure data is not empty
    ...
    if($product->addProduct())
    {
        ...
        $current_directory=$current_directory.trim($_POST['product_sku'],'"')."/";

        if(!is_dir($current_directory))
        mkdir($current_directory,0777);

        http_response_code(200);
        echo json_encode(array("message"=>"True","product_id"=>$product->product_id,"product_SKU"=>$_POST['product_sku']));
    }
